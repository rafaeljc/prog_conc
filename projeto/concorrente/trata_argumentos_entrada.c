...
void imprime_msg_argumento_invalido(char* arquivo) {
  ...
  fprintf(stderr, "Uso: %s <matriz A> <vetor b> [-max_iter <num>] [-e <num>] [-num_threads <num>]\n", arquivo);
  ...
  if (argc > 3) {    
    for (int i = 3; i < argc; i += 2) {
      ...
      if (strcmp(argv[i], "-max_iter") == 0) {
        ...
        if (max_iter < 1) imprime_msg_argumento_invalido(argv[0]);
        continue;
      }
      if (strcmp(argv[i], "-e") == 0) {
        e = atof(argv[i + 1]);
        if (!(e > 0.0)) imprime_msg_argumento_invalido(argv[0]);
        continue;
      }
      if (strcmp(argv[i], "-num_threads") == 0) {
        num_threads = atoi(argv[i + 1]);
        if (num_threads < 1) imprime_msg_argumento_invalido(argv[0]);
        continue;
      }
